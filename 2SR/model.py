...
AWAN=net.AWAN
net=importlib.import_module('models.2020zhang_sSR')
FMNet=net.FMNet
...
def model_generator(method, pretrained_model_path=None):
    if method == 'DenseUnet':
        model = DenseU(in_channel=3,out_channel=31)
    elif method == 'CanNet':
        model = Cannet(in_channel=3,out_channel=31)
    elif method == 'HSCNN+':
        model = hscnn_plus(38,in_channels=3, out_channels=31)
    elif method == 'sRCNN':
        model = sRCNN(in_channel=3, out_channel=31,L=16,F=128,K=3,P=1)
    elif method == 'AWAN':
        model = AWAN(inplanes=3, planes=31,reduction=8)
    elif method == 'FMNet':
        model = FMNet(bNum=3, nblocks=4, input_features=31, num_features=64, out_features=31)
    elif method == 'HRNet':
        model = SGN(in_channels=3, out_channels=31)
    elif method =='HSRnet':
        model=HSRnet(3,31)
    elif method == 'HSACS':
        model =HSACS(inplanes=3, planes=31)
    elif method =='GDNet':
        model=GDNet(in_ch=3,out_ch=31)
    elif method =='SSDCN':
        model=SSDCN(in_channels=3, out_channels=31)
    else:
