...
class FMNet(nn.Module):
    def __init__(self, init=True, bNum=3, nblocks=4, input_features=31, num_features=64, out_features=31):
        ...
        self.in_channel=input_features
        self.out_channel = out_features
        self.x1 = np.linspace(0, 1, num=input_features)
        self.x2 = np.linspace(0, 1, num=out_features)
    ...
    def forward(self, x):
        # data spectral resampling
        x = x.permute(2, 3, 1, 0)
        h,w,c,b=x.shape
        xnew = F.interpolate(x, (self.out_channel,b), mode='bilinear')
        xnew = xnew.permute(3, 2, 0, 1)

        fea_0 = self.input(xnew)
        fea = fea_0
