...
# Training
parser = argparse.ArgumentParser(description="DL4sSR Spectral Compressive Imaging Toolbox")
...
def train(training_data_loader, mask3d_batch,optimizer, model, criterion, epoch, accumulation_steps, loss_vaule,scheduler):
    ...
    for iteration, batch in enumerate(training_data_loader, 0):
        ...
        output = model(input[:,:,1:16,1:16])

        loss = criterion(output, target[:,:,1:16,1:16])
        ssim = SSIMloss(output, target[:,:,1:16,1:16])
        psnr = PSNR(output, target[:,:,1:16,1:16])
